...
if not depsonly:
    print("Device %s not found. Attempting to retrieve device repository from WingsOS Github (http://github.com/WingsOS)." % device)
...
def is_in_manifest(projectpath):
    ...
    # ... and don't forget the wings snippet
    try:
        lm = ElementTree.parse(".repo/manifests/snippets/wings.xml")
        lm = lm.getroot()
    except:
        lm = ElementTree.Element("manifest")
...
def add_to_manifest(repositories):
    ...
    for repository in repositories:
        ...
        if is_in_manifest(repo_target):
            print('WingsOS/%s already fetched to %s' % (repo_name, repo_target))
            continue

        project = ElementTree.Element("project", attrib = {
            "path": repo_target,
            "remote": "github",
            "name": "WingsOS/%s" % repo_name,
...
def fetch_dependencies(repo_path):
    ...
    dependencies_path = repo_path + '/wings.dependencies'
    syncable_repos = []
    verify_repos = []
...
def get_default_or_fallback_revision(repo_name):
    ...
    githubreq = urllib.request.Request("https://api.github.com/repos/WingsOS/" + repo_name + "/branches")
    add_auth(githubreq)
...
print("Repository for %s not found in the WingsOS Github repository list. If this is in error, you may need to manually add it to your local_manifests/roomservice.xml." % device)
