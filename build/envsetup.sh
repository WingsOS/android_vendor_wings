...
cat <<EOF
Additional WingsOS functions:
- cout:            Changes directory to out.
- mmp:             Builds all of the modules in the current directory and pushes them to the device.
- mmap:            Builds all of the modules in the current directory and its dependencies, then pushes the package to the device.
- mmmp:            Builds all of the modules in the supplied directories and pushes them to the device.
- wingsgerrit:     A Git wrapper that fetches/pushes patch from/to WingsOS Gerrit Review.
- wingsrebase:     Rebase a Gerrit change and push it again.
- wingsremote:     Add git remote for WingsOS Gerrit Review.
...
function breakfast()
{
    target=$1
    local variant=$2
    source ${ANDROID_BUILD_TOP}/vendor/wings/vars/aosp_target_release
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/wings-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
...
function wingsremote()
{
    ...
    git remote rm wings 2> /dev/null
    ...
    local WINGS_USER=$(git config --get review.review.wingsos.org.username)
    if [ -z "$WINGS_USER" ]
    then
        git remote add wings ssh://review.wingsos.org:29418/$PFX$PROJECT
    else
        git remote add wings ssh://$WINGS_USER@review.wingsos.org:29418/$PFX$PROJECT
    fi
    echo "Remote 'wings' created"
}
...
function githubremote()
{
    ...
    git remote add github https://github.com/WingsOS/$PROJECT
...
function makerecipe() {
    ...
    then
        pwd
        wingsremote
        git push wings HEAD:refs/heads/'$1'
    fi
    '
}

function wingsgerrit() {
    ...
    local user=`git config --get review.review.wingsos.org.username`
...
function wingsrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "WingsOS Gerrit Rebase Usage: "
        echo "      wingsrebase <path to project> <patch IDs on Gerrit>"
    ...
    git fetch "http://review.wingsos.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $WINGS_FIXUP_COMMON_OUT ]; then
